...
class Dimensioned(LabelledData):
    ...
    * deep dimensions
    ...
    hover_fields = param.List(
        default=None,
        doc="""
        List of fields to display in hover/tooltip. Each item can be:
        - A dimension name (string)
        - A Dimension object
        - A dim() expression for derived/computed fields
        - A special dimension name (e.g. 'x', 'y', 'z' for coordinates)
        
        If None, defaults to all key and value dimensions.
        """,
    )
    ...
    hover_formatters = param.Dict(
        default={},
        doc="""
        Dictionary mapping field names (or dimension names) to formatter
        functions or format strings for display in hover/tooltip.
        """,
    )

    hover_aliases = param.Dict(
        default={},
        doc="""
        Dictionary mapping field names (or dimension names) to display
        aliases/labels shown in hover/tooltip.
        """,
    )
    ...
    def _get_hover_field_label(self, field):
        ...
        if field_name in self.hover_aliases:
            return self.hover_aliases[field_name]

        if isinstance(field, dim) and str(field) in self.hover_aliases:
            return self.hover_aliases[str(field)]
    ...
    def _get_hover_field_values(self, field, expanded=True, flat=True):
        ...
        if isinstance(field, dim):
            return field.apply(self, flat=flat)
        elif isinstance(field, Dimension):
            return self.dimension_values(field.name, expanded=expanded, flat=flat)
        elif isinstance(field, str):
            existing_dim = self.get_dimension(field)
            if existing_dim is not None:
                return self.dimension_values(existing_dim.name, expanded=expanded, flat=flat)
            if field in self.cdims:
                return np.array([self.cdims[field]])
            raise KeyError(f"Field '{field}' not found in element dimensions.")
        else:
    ...
    def _get_hover_formatter(self, field):
        ...
        if field_name in self.hover_formatters:
            return self.hover_formatters[field_name]
