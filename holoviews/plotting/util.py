...
_RESERVED_FIELD_PREFIX = "dim_"
_RESERVED_INTERNAL_FIELDS = {
    "x", "y", "z", "xs", "ys", "zs",
    "color", "size", "line_color", "fill_color",
    "marker", "line_width", "alpha", "angle",
    "width", "height", "radius",
    "image", "dw", "dh",
    "__angle", "__x", "__y", "__z",
}
...
class DimensionFieldMap:
    """Bidirectional dimension↔field mapping with conflict resolution and
    reserved-name protection.

    Each plot (or frame in a DynamicMap/HoloMap) builds its own
    DimensionFieldMap so that every dimension in every element of an
    Overlay, NdOverlay, DynamicMap, or HoloMap maps to a unique,
    sanitized data-source column name. Collisions between two distinct
    dimensions that sanitize to the same identifier are resolved with a
    numeric suffix, and identifiers that clash with reserved internal
    field names (x, y, color, size, …) are automatically prefixed with
    ``dim_``.

    The map is bidirectional: use :meth:`field_for` to look up the column
    name for a dimension, and :meth:`dimension_for` to recover the
    original Dimension object from a column name.

    Examples
    --------
    >>> from holoviews import Points, dim
    >>> from holoviews.plotting.util import DimensionFieldMap
    >>> pts = Points({'X Value': [1,2], 'Y/Value': [3,4]})
    >>> fm = DimensionFieldMap.from_element(pts)
    >>> fm.field_for('X Value')
    'X_Value'
    >>> fm.field_for('Y/Value')
    'Y_over_Value'
    >>> fm.dimension_for('X_Value').name
    'X Value'
    """
    ...
    def __init__(self, reserved=None):
        ...
        self._field_to_dim = {}
        self._dim_to_label = {}
        self._reserved = set(_RESERVED_INTERNAL_FIELDS)
        if reserved:
            self._reserved.update(reserved)

    # ------------------------------------------------------------------
    # Construction helpers
    # ------------------------------------------------------------------
    @classmethod
    def from_element(cls, element, reserved=None):
        """Build a mapping from a single Element, collecting its kdims,
        vdims, and any overlay dimensions."""
        fm = cls(reserved=reserved)
        fm._register_element(element)
        return fm

    @classmethod
    def from_objects(cls, objects, reserved=None):
        """Build a mapping by collecting every dimension from an iterable
        of Elements, Overlays, HoloMaps, or DynamicMaps."""
        fm = cls(reserved=reserved)
        for obj in objects:
            fm.register(obj)
        return fm
    ...
    def register(self, obj):
        """Recursively register all dimensions from *obj*, which may be
        an Element, Overlay, NdOverlay, HoloMap, or DynamicMap."""
        ...
        if isinstance(obj, DynamicMap):
            ...
            for v in obj.data.values():
                self.register(v)
        elif isinstance(obj, (CompositeOverlay, NdOverlay, Overlay)):
            for v in obj:
                self.register(v)
        elif isinstance(obj, Element):
            self._register_element(obj)

    def _register_element(self, element):
        from ..core.dimension import Dimension

        all_dims = list(element.dimensions())
        # Overlay dimensions held on the plot instance
        overlay_dims = getattr(element, "_overlay_dims", {})
        for od in overlay_dims.keys():
            if isinstance(od, Dimension) and od not in all_dims:
                all_dims.append(od)
        ...
        for d in all_dims:
            ...
            if not isinstance(d, Dimension):
                d = element.get_dimension(d) or Dimension(str(d))
            key = d.name
            if key in self._dim_to_field:
                continue
            field = self._resolve_field(d)
            self._dim_to_field[key] = field
            self._field_to_dim[field] = d
            self._dim_to_label[key] = d.pprint_label

    def _resolve_field(self, dimension):
        """Return a unique, sanitized field name for *dimension*, handling
        collisions with previously-registered dimensions and with the
        reserved internal field-name set."""
        from ..core.dimension import Dimension

        base = dimension_sanitizer(
            dimension.name if isinstance(dimension, Dimension) else str(dimension)
        )
        if base in self._reserved:
            base = _RESERVED_FIELD_PREFIX + base
        candidate = base
        counter = 2
        while candidate in self._field_to_dim and self._field_to_dim[candidate] is not dimension:
    ...
    # Lookup API
    # ------------------------------------------------------------------
    def field_for(self, dimension):
        """Return the canonical data-source field name for *dimension*.

        Accepts a dimension name string, a Dimension object, or a dim()
        transform. If the dimension has not yet been registered it is
        ...
        if key not in self._dim_to_field:
            d = Dimension(key)
            self._register_standalone(d)
        return self._dim_to_field[key]

    def label_for(self, dimension):
        """Return the human-readable display label for *dimension*."""
        from ..core.dimension import Dimension
        ...
        if key not in self._dim_to_label:
            d = Dimension(key)
            self._register_standalone(d)
        return self._dim_to_label[key]

    def dimension_for(self, field):
        """Return the original Dimension object corresponding to *field*,
        or ``None`` if the field is unknown."""
    ...
    # ------------------------------------------------------------------
    def as_dict(self):
        """Return the forward dimension-name → field-name mapping as a plain
        dict (useful for serialisation / export)."""
        return dict(self._dim_to_field)

    def fields(self):
        """Return all registered field names."""
        return list(self._field_to_dim.keys())

    def dimension_names(self):
        """Return all registered original dimension names."""
        return list(self._dim_to_field.keys())

    def labels(self):
        """Return all registered human-readable labels."""
        return list(self._dim_to_label.values())

    # ------------------------------------------------------------------
    # Internal
    # ------------------------------------------------------------------
    def _register_standalone(self, dimension):
        key = dimension.name
        if key in self._dim_to_field:
            return
        field = self._resolve_field(dimension)
        self._dim_to_field[key] = field
        self._field_to_dim[field] = dimension
        self._dim_to_label[key] = dimension.pprint_label

    def __repr__(self):
        items = ", ".join(
            f"{k!r}: {v!r}" for k, v in self._dim_to_field.items()
        )
        return f"DimensionFieldMap({{{items}}})"
