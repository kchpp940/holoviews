...
class BaseNarwhalsInterfaceTests(HeterogeneousColumnTests, InterfaceTests):
    ...
    def test_narwhals_categorical_groupby_order(self):
        if self.narwhals_backend != "polars":
            pytest.skip("polars-specific categorical test")
        df = pl.DataFrame({
            "cat": pl.Series(["b", "a", "b", "a"]).cast(pl.Categorical),
            "val": [10, 20, 30, 40],
        })
        ds = hv.Dataset(nw.from_native(df), kdims=["cat"], vdims=["val"], datatype=["narwhals"])
        grouped = ds.groupby("cat")
        keys = list(grouped.keys())
        assert keys[0] == "b" and keys[1] == "a"

    def test_narwhals_categorical_aggregate_order(self):
        if self.narwhals_backend != "polars":
            pytest.skip("polars-specific categorical test")
        df = pl.DataFrame({
            "cat": pl.Series(["b", "a", "b", "a"]).cast(pl.Categorical),
    ...
    def test_narwhals_tz_aware_datetime_values(self):
        if self.narwhals_backend not in ("pandas", "polars"):
            pytest.skip("tz-aware datetime not supported by this backend")
        if self.narwhals_backend == "polars":
            df = pl.DataFrame({
                "x": pl.Series(["2020-01-01", "2020-01-02", "2020-01-03"]).str.to_datetime(time_zone="UTC"),
                "y": [1, 2, 3],
            })
        else:
            import pandas as pd
            df = pd.DataFrame({
                "x": pd.date_range("2020-01-01", periods=3, freq="D", tz="UTC"),
                "y": [1, 2, 3],
            })
        ds = hv.Dataset(nw.from_native(df), kdims=["x"], vdims=["y"], datatype=["narwhals"])
        vals = ds.dimension_values("x")
        assert vals.dtype == np.dtype("datetime64[ns]")

    def test_narwhals_tz_aware_datetime_range(self):
        if self.narwhals_backend not in ("pandas", "polars"):
            pytest.skip("tz-aware datetime not supported by this backend")
        if self.narwhals_backend == "polars":
            df = pl.DataFrame({
        ...
        ds = hv.Dataset(nw.from_native(df), kdims=["x"], vdims=[], datatype=["narwhals"])
        rng = ds.range("x")
        assert rng[0] == datetime(2020, 1, 1)
        assert rng[1] == datetime(2020, 1, 3)

    def test_narwhals_lazyframe_range(self):
        if not isinstance(self.data_type, tuple) and not issubclass(self.data_type, nw.LazyFrame if hasattr(nw, "LazyFrame") else object):
            pytest.skip("Only LazyFrame backends")
        ds = self.dataset_hm
        if not isinstance(ds.data, nw.LazyFrame):
            pytest.skip("Not a LazyFrame")
        rng = ds.range("x")
        assert rng == (0, 10)

    def test_narwhals_lazyframe_values(self):
        if not isinstance(self.data_type, tuple) and not issubclass(self.data_type, nw.LazyFrame if hasattr(nw, "LazyFrame") else object):
            pytest.skip("Only LazyFrame backends")
        ds = self.dataset_hm
        if not isinstance(ds.data, nw.LazyFrame):
    ...
    def test_narwhals_lazyframe_length(self):
        if not isinstance(self.data_type, tuple) and not issubclass(self.data_type, nw.LazyFrame if hasattr(nw, "LazyFrame") else object):
            pytest.skip("Only LazyFrame backends")
        ds = self.dataset_hm
        if not isinstance(ds.data, nw.LazyFrame):
