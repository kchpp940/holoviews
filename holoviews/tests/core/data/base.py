...
class GriddedInterfaceTests:
    ...
    def test_dataset_range_with_nan_float(self):
        ds = hv.Dataset(
            self.frame({"x": [1.0, 2.0, np.nan, 4.0, np.nan]}), kdims=["x"], vdims=[]
        )
        assert ds.range("x") == (1.0, 4.0)

    def test_dataset_sort_nulls_last(self):
        ds = hv.Dataset(
            self.frame({"x": [3, np.nan, 1, np.nan, 2], "y": [5, 6, 7, 8, 9]}),
            kdims=["x"], vdims=["y"],
        )
        sorted_ds = ds.sort("x")
        vals = sorted_ds.dimension_values("x")
        assert np.isnan(vals[-1]) and np.isnan(vals[-2])
        assert vals[0] == 1 and vals[1] == 2 and vals[2] == 3

    def test_dataset_values_returns_ndarray(self):
        vals = self.table.dimension_values("Gender")
        assert isinstance(vals, np.ndarray)
        vals_y = self.table.dimension_values("Height")
        assert isinstance(vals_y, np.ndarray)

    def test_dataset_values_expanded_false_returns_ndarray(self):
        vals = self.table.dimension_values("Gender", expanded=False)
        assert isinstance(vals, np.ndarray)

    def test_dataset_datetime_range(self):
        dates = [
            datetime.datetime(2020, 1, 1),
            datetime.datetime(2020, 1, 2),
            datetime.datetime(2020, 1, 3),
        ]
        ds = hv.Dataset(self.frame({"x": dates, "y": [1, 2, 3]}), kdims=["x"], vdims=["y"])
        rng = ds.range("x")
        assert rng == (dates[0], dates[-1])

    def test_dataset_datetime_range_with_nat(self):
        dates = [
            datetime.datetime(2020, 1, 1),
            None,
            datetime.datetime(2020, 1, 3),
        ...
        ds = hv.Dataset(self.frame({"x": dates, "y": [1, 2, 3]}), kdims=["x"], vdims=["y"])
        rng = ds.range("x")
        assert rng[0] == datetime.datetime(2020, 1, 1)
        assert rng[1] == datetime.datetime(2020, 1, 3)

    def test_dataset_select_datetime_string(self):
        dates = [
            datetime.datetime(2020, 1, 1),
            datetime.datetime(2020, 1, 2),
            datetime.datetime(2020, 1, 3),
        ...
        ds = hv.Dataset(self.frame({"x": dates, "y": [1, 2, 3, 4, 5]}), kdims=["x"], vdims=["y"])
        selected = ds.select(x=("2020-01-02", "2020-01-05"))
        assert len(selected) == 3

    def test_dataset_dimension_type_int(self):
        ds = hv.Dataset(self.frame({"x": [1, 2, 3], "y": [4, 5, 6]}), kdims=["x"], vdims=["y"])
        assert ds.interface.dimension_type(ds, "x") in (int, np.integer, np.int64)

    def test_dataset_dimension_type_float(self):
        ds = hv.Dataset(self.frame({"x": [1.0, 2.0, 3.0], "y": [4, 5, 6]}), kdims=["x"], vdims=["y"])
        assert ds.interface.dimension_type(ds, "x") in (float, np.floating, np.float64)

    def test_dataset_categorical_groupby_order(self):
        ds = hv.Dataset(
            self.frame({"cat": ["b", "a", "b", "a"], "val": [10, 20, 30, 40]}),
            kdims=["cat"], vdims=["val"],
        )
        grouped = ds.groupby("cat")
        keys = list(grouped.keys())
        assert keys[0] == "b" and keys[1] == "a"

    def test_dataset_categorical_aggregate_order(self):
        ds = hv.Dataset(
            self.frame({"cat": ["b", "a", "b", "a"], "val": [10, 20, 30, 40]}),
            kdims=["cat"], vdims=["val"],
        )
        ...
        assert cat_vals[0] == "b" and cat_vals[1] == "a"

    def test_dataset_values_datetime_resolution(self):
        dates = [
            datetime.datetime(2020, 1, 1),
            datetime.datetime(2020, 1, 2),
            datetime.datetime(2020, 1, 3),
