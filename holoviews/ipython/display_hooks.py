...
from ..plotting.artifact_manager import (
    ArtifactKind,
    CleanupPolicy,
    artifact_manager,
)
...
def render(obj, **kwargs):
    ...
    with artifact_manager.default_owner("ipython_display_hook"):
        with artifact_manager.scope(f"notebook-render-{id(obj)}") as scope:
...
def first_frame(obj):
    ...
    with artifact_manager.default_owner("ipython_display_hook"):
        artifact_manager.register_data(
            ...
            owner="display_hook",
            policy=CleanupPolicy.RENDER_CYCLE,
            refs={"obj_id": id(obj), "hook": "first_frame"},
        )
...
def middle_frame(obj):
    ...
    with artifact_manager.default_owner("ipython_display_hook"):
        artifact_manager.register_data(
            ...
            owner="display_hook",
            policy=CleanupPolicy.RENDER_CYCLE,
            refs={"obj_id": id(obj), "hook": "middle_frame"},
        )
    return {"text/html": html}
...
def last_frame(obj):
    ...
    with artifact_manager.default_owner("ipython_display_hook"):
        artifact_manager.register_data(
            ...
            owner="display_hook",
            policy=CleanupPolicy.RENDER_CYCLE,
            refs={"obj_id": id(obj), "hook": "last_frame"},
        )
    return {"text/html": html}
...
def display_hook(fn):
    ...
    All artifacts generated by the wrapped hook are registered under a
    dedicated artifact scope so they can be tracked and cleaned up
    uniformly.
    ...
    def wrapped(element):
        ...
        with artifact_manager.default_owner(f"display_hook:{fn.__name__}"):
            with artifact_manager.scope(f"nbhook-{fn.__name__}-{id(element)}") as scope:
                try:
